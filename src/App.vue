<template>
  <div class="flex flex-col h-screen p-5 font-body space-y-5 container mx-auto">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3">
      <!-- Card -->
      <div v-for="(p, i) of pokemons" :key="i" class="flex flex-col col-span-1 rounded-2xl p-4 border-2 border-gray-600 hover:shadow-lg hover:-mt-2 cursor-pointer" :class="`bg-${p.color}-500`">
        <span>{{ p.name }}</span>
  ...
  created(){
    ...
    Promise.all([getPokemonColors(), getPokemons()])
      .then(async (res) => {
        ...
        const pokemons = res[1].data.results
